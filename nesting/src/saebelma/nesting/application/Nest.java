...
import saebelma.nesting.io.PolygonParser;
import saebelma.nesting.irregular.IrregularShapeNesting;
import saebelma.nesting.packing.BoxPacking;
import saebelma.nesting.packing.CirclePacking;
import saebelma.nesting.svg.SVG;
import saebelma.nesting.svg.SVGGroup;
import saebelma.nesting.svg.SVG.StrokeWidth;
import saebelma.nesting.svg.SVG.Style;
import saebelma.nesting.tuple.TupleNesting;

/**
 * A simple command line application demonstrating various packing and saebelma.nesting algorithms. By
 ...
 * enclosing circle) and of several packing and saebelma.nesting results.
 */
public class Nest {
    ...
    public static void main(String[] args) throws IOException {
        ...
        IrregularShapeNesting irregularShapeNesting = new IrregularShapeNesting(
                polygon);
        System.out.println(
                "Irregular shape saebelma.nesting with convex hull criterion in "
        ...
        System.out.println(
                "Irregular shape saebelma.nesting with smallest enclosing circle criterion in "
                        + (System.currentTimeMillis() - startTime) + " ms (n = "
                        + irregularShapeNesting.getN() + ")");
        ...
        System.out.println("Tuple saebelma.nesting type 1 (convex hull) in "
                + (System.currentTimeMillis() - startTime) + " ms (n = "
                + tupleNesting.getN() + ")");
        ...
        System.out
                .println("Tuple saebelma.nesting type 2 (smallest enclosing circle) in "
                        + (System.currentTimeMillis() - startTime) + " ms (n = "
                        + tupleNesting.getN() + ")");
